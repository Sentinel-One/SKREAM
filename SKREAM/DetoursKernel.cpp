...
#pragma warning(suppress:6011) // GetModuleHandleW(NULL) never returns NULL.
        ...
        PVOID* iidp = (PVOID*)RvaAdjust(pDosHeader, pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress);

        if (iidp == NULL) {
            return STATUS_INVALID_IMAGE_FORMAT;
        }

        auto size = pNtHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].Size;
        PVOID endOfIat = iidp + (size / sizeof(PVOID));

        for (; iidp < endOfIat; iidp++) {
            if (iidp != NULL) {
                if (!pfImportFunc(iidp)) {
                    break;
                }
            }
        }
